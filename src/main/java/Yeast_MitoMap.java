...
private ArrayList<Integer> roiZ;
private double[] MVarea = new double[16];
private static final Font labelFont = new Font("Helvetica",Font.BOLD,12);
private static final double ADJACENT = 2d;	//max distance to an adjacent pixel. 1,1,1 (1.732) is distance to vertex-connected neighbour
...
//if(1==1){map.show();return;}	//debugging
	...
	for(int i=0;i<id.length;i++){
		...
		ref++;
		Prefs.blackBackground = true;
		ArrayList<Point3d> vox = new ArrayList<Point3d>();
		ArrayList<Point3d> voxCal = new ArrayList<Point3d>();
			...
			for(int y=0;y<objH;y++){
				for(int x=0;x<objW;x++){
					if(obj.getPixel(x,y)[0]==255){
						vox.add(new Point3d(x,y,z));
						voxCal.add(new Point3d(x*cal.pixelWidth,y*cal.pixelHeight,z*cal.pixelDepth));
						centroidX += x;
						centroidY += y;
						centroidZ += z;
	...
	//IJ.log("= "+mean);
		...
		Point3d centroidCal = new Point3d(centroidX*cal.pixelWidth,centroidY*cal.pixelHeight,centroidZ*cal.pixelDepth);
		
		ArrayList<Point3d> surface = new ArrayList<Point3d>();
		ArrayList<Point3d> surfaceCal = new ArrayList<Point3d>();
		...
		for(int a=0;a<vox.size();a++){
			...
			if(Hcount+Vcount>0){	//if this voxel has exposed faces
				...
				surfaceCal.add(new Point3d(new Point3d(vox.get(a).x*cal.pixelWidth,vox.get(a).y*cal.pixelHeight,vox.get(a).z*cal.pixelDepth)));
				int MVclass = -1;
		...
		for(int v=0;v<vox.size();v++){
			...
			meanDtoXaxis += voxCal.get(v).distance(new Point3d(voxCal.get(v).x,centroidCal.y,centroidCal.z));
			meanDtoYaxis += voxCal.get(v).distance(new Point3d(centroidCal.x,voxCal.get(v).y,centroidCal.z));
			meanDtoZaxis += voxCal.get(v).distance(new Point3d(centroidCal.x,centroidCal.y,voxCal.get(v).z));
		}
		...
		for(int v=0;v<vox.size();v++){
			...
			varDtoXaxis += Math.pow(voxCal.get(v).distance(new Point3d(voxCal.get(v).x,centroidCal.y,centroidCal.z))-meanDtoXaxis,2);
			varDtoYaxis += Math.pow(voxCal.get(v).distance(new Point3d(centroidCal.x,voxCal.get(v).y,centroidCal.z))-meanDtoYaxis,2);
			varDtoZaxis += Math.pow(voxCal.get(v).distance(new Point3d(centroidCal.x,centroidCal.y,voxCal.get(v).z))-meanDtoZaxis,2);
		}
